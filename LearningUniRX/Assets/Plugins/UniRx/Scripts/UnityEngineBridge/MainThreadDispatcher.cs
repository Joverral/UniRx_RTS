...
namespace UniRx
{
    public sealed class MainThreadDispatcher : MonoBehaviour
    {
            ...
            else
            {
                ...
                if (!isQuitting && !object.ReferenceEquals(dispatcher, null))
                {
                    dispatcher.queueWorker.Enqueue(_ =>
                    {
                        ...
                        if (distpacher2 != null)
                        {
                            var behave = (MonoBehaviour)distpacher2;
                            behave.StartCoroutine(routine);
                            //TODO:WTF?
        ...
        new public static Coroutine StartCoroutine(IEnumerator routine)
        {
            ...
            if (dispatcher != null)
            {
                var behave = (MonoBehaviour)dispatcher;
                behave.StartCoroutine(routine);
                return null; // TODO wtf dispatcher.StartCoroutine(routine);
            }
            else
            {
                return null;
